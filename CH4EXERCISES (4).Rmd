...
```{r}
cars20to30 <- filter(select(simplecars, carname, mpg), mpg>20 & mpg<30)
cars20to30

```
Ensure this cars20to30 dataframe's data is sorted from best to worst in miles per gallon.  Show you
```{r}
...
```{r}
cylmpg<- summarize(group_by(mycars, cyl, carname), mean(mpg))
cylmpg
ggplot (data= cylmpg,mapping= aes(x= carname, y= cyl))+
...
carps <- select(carps, hp, carname, powerspeed, qsec)
carps%>%arrange(mycars, desc(powerspeed))
carps
```
